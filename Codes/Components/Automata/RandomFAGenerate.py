...
class RandomAutomatonGenerator:
    def __init__(self, extra_branch_prob=0.1, max_extra_states=3, loop_prob=0.1, max_branch=3):
        """
    ...
    def generate(self, input_string):
        ...
        transitions : dict = {s: {} for s in states}
        ...
        for s in all_states:
            for c in alphabet:
                if random.random() < self.extra_branch_prob:
                    if len(transitions[s]) > self.max_branch: break
        ...
        # ----------------------------------
        # 6. Xóa các state dư thừa (unreachable, no outgoing, only self-loop)
        # ----------------------------------
        while True:
        ...
        # ----------------------------------
        # 7. XÓA UNREACHABLE STATES (không thể đến từ start_state)
        # ----------------------------------
        reachable_states = self._find_reachable_states(start_state, transitions)
        unreachable = [s for s in all_states if s not in reachable_states and s not in accept_states]
        
        for state in unreachable:
            self.remove_state_from_transitions(transitions, [], state)
            all_states.remove(state)
            print(f"From FAGenerator: Removed {state} 'cause it's unreachable from start state")
        
        print("FA after removed!")
        print(f"States: {all_states}")
        print(f"Alphabet: {alphabet}")
        for state in transitions:
            print(f"{state}: {dict(transitions[state].items())}")
    ...
    def _find_reachable_states(self, start_state, transitions):
        """
        BFS để tìm tất cả state có thể đến được từ start_state
        """
        reachable = set()
        ...
        while queue:
            ...
            # Duyệt tất cả next states từ current
            if current in transitions:
                for char, next_states in transitions[current].items():
    ...
    def remove_state_from_transitions(self, transitions, removed_states, state):
        ...
        self._remove_from_to_list(transitions, state)
        if state in transitions:
            transitions.pop(state)

    def _remove_from_to_list(self, transitions, state):
